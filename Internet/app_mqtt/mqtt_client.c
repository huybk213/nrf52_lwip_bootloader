...
#ifdef MQTT_WITH_SSL
//TLS
static mbedtls_entropy_context m_ssl_entropy_ctx;
static mbedtls_ctr_drbg_context m_ssl_drbg_ctx;
static mbedtls_ssl_context m_ssl_ctx;
static mbedtls_ssl_config m_ssl_conf;
static mbedtls_x509_crt m_x509_root_ca;
static mbedtls_x509_crt m_x509_client_key;
static mbedtls_pk_context m_pk_private_key;
static mbedtls_ctr_drbg_context m_ssl_drbg_ctx;
...
{ 
    ...
    mbedtls_ctr_drbg_free(&m_ssl_drbg_ctx);
    mbedtls_entropy_free(&m_ssl_entropy_ctx);
}
...
static int tls_init(void)
{
    ...
    mbedtls_x509_crt_init(&m_x509_root_ca);
    mbedtls_x509_crt_init(&m_x509_client_key);
    mbedtls_pk_init(&m_pk_private_key);

    mbedtls_ctr_drbg_init(&m_ssl_drbg_ctx);
    mbedtls_entropy_init(&m_ssl_entropy_ctx);

    if ((ret = mbedtls_ctr_drbg_seed(&m_ssl_drbg_ctx, mbedtls_entropy_func, &m_ssl_entropy_ctx,
     ...
     */
    if ((ret = mbedtls_ssl_config_defaults(&m_ssl_conf,
    ...
    ret = mbedtls_x509_crt_parse(&m_x509_root_ca,
    ...
    ret = mbedtls_x509_crt_parse(&m_x509_client_key,
                                 (const unsigned char *)aws_certificate_get_client_cert(),
                                 strlen(aws_certificate_get_client_cert()) + 1);
    if (ret != 0)
    {
        DebugPrint("Parse m_x509_client_key error -0x%04X\r\n", ret);
        return -1;
    }

    ret = mbedtls_pk_parse_key(&m_pk_private_key,
                               (const unsigned char *)aws_certificate_get_client_key(),
                               strlen(aws_certificate_get_client_key()) + 1, NULL, 0);
    if (ret != 0)
    {
        DebugPrint("Parse m_pk_private_key error -0x%04X\r\n", ret);
        return -1;
    }

    DebugPrint("Parse certificate success\r\n");
    mbedtls_ssl_conf_ca_chain(&m_ssl_conf, &m_x509_root_ca, NULL);
    mbedtls_ssl_conf_authmode(&m_ssl_conf, MBEDTLS_SSL_VERIFY_REQUIRED);
    //    mbedtls_ssl_conf_verify(&m_ssl_conf, (int (*)(void *, mbedtls_x509_crt *, int, uint32_t *))mqtt_tls_verify, NULL );
    ...
    mbedtls_ssl_conf_rng(&m_ssl_conf, mbedtls_ctr_drbg_random, &m_ssl_drbg_ctx);
    mbedtls_ssl_conf_dbg(&m_ssl_conf, mbedtls_alt_debug, stdout);

    //    mbedtls_ssl_setup(&m_ssl_ctx, &m_ssl_conf);

    if (ret = mbedtls_ssl_set_hostname(&m_ssl_ctx, aws_get_arn()) != 0)
    {
    ...
    //    mbedtls_ssl_set_bio(&m_ssl_ctx, &mqtt_static_client, altcp_mbedtls_bio_send, altcp_mbedtls_bio_recv, NULL);
